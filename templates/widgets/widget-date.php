		...
		<div class="hms-widget-info-datetime">
			<?php
				if ( hms_get_option( 'event_start_timestamp' ) ) {
					echo esc_html( sprintf( __( '%s on %s', 'hackathon' ), hms_date( hms_get_option( 'event_start_timestamp' ), 'F j' ), hms_date( hms_get_option( 'event_start_timestamp' ), 'H:i' ) ) );
				} else {
		...
		<div class="hms-widget-info-datetime">
			<?php
				if ( hms_get_option( 'event_end_timestamp' ) ) {
					echo esc_html( sprintf( __( '%s on %s', 'hackathon' ), hms_date( hms_get_option( 'event_end_timestamp' ), 'F j' ), hms_date( hms_get_option( 'event_end_timestamp' ), 'H:i' ) ) );
				} else {
					esc_html_e( 'Date not specified.', 'hackathon' );
				}
			?>
